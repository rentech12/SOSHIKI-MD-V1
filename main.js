...
// Command imports
const Command = require('./commands/tagall');
...
const { Command, handleJoinEvent } = require('./commands/');
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        const ownerCommands = ['.mode', '.autostatus', '.antidelete', '.cleartmp', '.setpp', '.clearsession', '.areact', '.newsletter', '.autoreact', '.autotyping', '.autoread', '.pmblocker'];
...
case userMessage.startsWith('.channelinfo'):
    {
        ...
        const extra = { from: chatId, reply: (text) => sock.sendMessage(chatId, { text, ...channelInfo }, { quoted: message }) };
        await newsletterCommand.execute(sock, message, args, extra);
    }
    commandExecuted = true;
    break;
